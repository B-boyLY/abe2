    ...
    for(field in fields) {
      switch field.kind {
        case FFun(f): f.args = f.args.concat([
            { name : "request", type : macro : Dynamic, value : null, opt : false },
            { name : "response", type : macro : Dynamic, value : null, opt : false },
            { name : "next", type : macro : Dynamic, value : null, opt : false }
          ]);
        case _: continue;
      }
    }
  ...
  static function filterControllerMethods(fields : Array<Field>) {
    ...
    for(field in fields) {
      ...
      for(meta in field.meta) {
        ...
        if (!Methods.list.any(function (method) return method == find)) {
          continue;
        }
        results.push(field);
        break;
